...
function timeCommand(receivedMessage = null, argument = "5:00"){
  ...
  if(argument == "5:00"){ // check whether the argument match time format 5:00
    ...
    .setDescription(argument + ` is approximately: `+ `${Math.floor(38336 / 1.2) } (UP: 38336)`)
    .setColor('#0099ff');
    receivedMessage.channel.send(timeMsg)
    return;
  }
  ...
  const timeMsg2 = new Discord.MessageEmbed()
  .setDescription(argument + ` is approximately: ` + `${score} (UP: ${up_score})`)
  .setColor('#0099ff');
  receivedMessage.channel.send(timeMsg2)
}
...
function scoreCommand(receivedMessage, argument) {
  if (score.toString().includes(argument)) {  // check if argument is in array score
      ...
      .setDescription("Time is approximately: " + `${times[arrIndex]}, where score is approximately: ` + `${score[arrIndex]}` + " (UP: " + `${UPscore[arrIndex]}` + ")")
      .setColor('#0099ff');
      receivedMessage.channel.send(scoreMsg)
    }
    ...
    const scoreUPmsg = new Discord.MessageEmbed()
    .setDescription("Time is approximately: " + `${times[arrIndex]}, where score is approximately: ` + `${score[arrIndex]}` + " (UP: " + `${UPscore[arrIndex]}` + ")")
    .setColor('#0099ff');
    receivedMessage.channel.send(scoreUPmsg)
  }
  else {
